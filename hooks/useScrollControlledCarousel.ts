...
export const useScrollControlledCarousel = ({
  ...
  const moveToSlide = useCallback((slideIndex: number) => {
    if (!emblaApiRef.current || isAnimatingRef.current || slideIndex < 0 || slideIndex >= itemsCount) return;
  ...
  const handleScrollEvent = useCallback((event: WheelEvent) => {
  ...
  }), [handleScrollEvent, isScrollLocked, isCarouselActive, currentSlide, itemsCount, moveToSlide]);
